...
export const Chat = ({ handleCurrConversation }: { handleCurrConversation: () => void }) => {
    ...
    const translationSetting = useSelector(
        (state: RootState) => state.translator.translationSetting[currConversationId || ''],
    );
    const notification = useSelector(
        (state: RootState) => state.conversation.notificationCount[currConversationId || ''],
    );
    ...
    const handleInputTranslate = async () => {
        ...
        try {
            ...
            const response = await axios.post(`${config.API_URL}/api/v1/translate/inputtext`, {
                text: content,
    ...
    return (
        <div className="chat">
            ...
            <div className="chat-navbar">
                ...
                <button onClick={() => setShowConversationSetting(!showConversationSetting)}>
                    <KebabMenu />
                </button>
            </div>
            <div className="messages">
                {messages.map((message, index) => (
                    <div key={message.id} ref={index === messages.length - 1 ? scrollRef : null}>

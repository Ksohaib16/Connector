...
const ChatManager = {
  ...
  attachEventListeners() {
    ...
    document
      .querySelector(".profile-close-overlay")
    ...
    document.getElementById("friendList")?.addEventListener("click", (e) => {
      if (e.target.closest(".friend")) {
        const conversationId =
          e.target.closest(".friend").dataset.conversationId;
      ...
      if (e.target.closest(".friend")) {
        let conversationId = e.target.closest(".friend").dataset.conversationId;
    ...
    window.addEventListener("click", (e) => {
      if (
        e.target.id === "chatSetting" ||
        (!e.target.closest("#slideOverlay") &&
          !e.target.closest("#chatSetting"))
      ) {
        this.closeOverlaySetting();
      }
      if (
        e.target.id === "profileSetting" ||
        (!e.target.closest("#profileOverlay") &&
          !e.target.closest("#profileSetting"))
      ) {
        this.closeProfileSetting();
      }
    });
  ...
  async deleteConversation(conversationId) {
    ...
    try {
      let res = await axios.delete("/api/conversations/delete", {
        data: { conversationId }
      });
      ...
      console.error("Detailed error information:", {
        ...
        status: error.response?.status,
         ...
         popupManager.showStatus("Conversation deleted successfuly")
    }
  },
  ...
  loadLanguageSelections() {
    ...
    if (languageSelections) {
      document.getElementById("userChoice").value =
        languageSelections.userChoice;
      document.getElementById("targetLanguage").value =
        languageSelections.targetLanguage;
    }
  },
  ...
  async createConversation(e) {
    ...
    if (friend) {
      ...
      searchContainer.innerHTML =
        '<div class="searchResult">Friend not found</div>'; // Show a not found message
  ...
  async quickTranslate(content, messageId, messageDiv) {
    ...
    const setupRevertButton = () => {
      quickTranslateElement.innerHTML =
